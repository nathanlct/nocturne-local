...
class RoadPoint : public geometry::AABBInterface {
 public:
  RoadPoint(geometry::Vector2D position, int type)
...
class RoadLine : public sf::Drawable {
 public:
  RoadLine(std::vector<geometry::Vector2D> geometry, RoadType road_type,
           bool checkForCollisions = false, int numPoints = 8);
  ...
  std::vector<std::shared_ptr<RoadPoint>>
  getRoadPoints();  // return an evenly spaced set of the geometry
 ...
 private:
  ...
  std::vector<std::shared_ptr<RoadPoint>>
      roadPoints;  // subsampled list of points
