...
class Simulation {
 public:
  Simulation(const std::string& scenario_path = "", int64_t start_time = 0,
             bool use_non_vehicles = true, bool spawn_invalid_objects = false)
      : scenario_path_(scenario_path),
        scenario_(std::make_unique<Scenario>(scenario_path, start_time,
                                             use_non_vehicles,
                                             spawn_invalid_objects)),
        start_time_(start_time),
        use_non_vehicles_(use_non_vehicles),
        spawn_invalid_objects_(spawn_invalid_objects) {}

  void Reset() {
    scenario_.reset(new Scenario(scenario_path_, start_time_, use_non_vehicles_,
                                 spawn_invalid_objects_));
  }
