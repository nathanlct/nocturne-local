...
np.testing.assert_allclose(new_state[0:num_object_states], [
    1, 10.0, 0.0, vehs[1].getLength(), vehs[1].getWidth(), -np.pi / 2, 5.0, 0,
    1, 0, 0, 0, 0, 0, 0
],
                           rtol=1e-5,
                           atol=1e-5)
...
np.testing.assert_allclose(stop_sign_state, [1, 8.0 * np.sqrt(2), -np.pi / 4],
                           rtol=1e-5,
                           atol=1e-5)
...
road_point_state = new_state[max_num_visible_objects *
                             num_object_states:max_num_visible_objects *
                             num_object_states + num_road_point_states * 2]
np.testing.assert_allclose(road_point_state, [0] * 2 * num_road_point_states,
                           rtol=1e-5,
                           atol=1e-5)
...
road_point_state = new_state[max_num_visible_objects *
                             ...
                             num_object_states + num_road_point_states * 2]
np.testing.assert_allclose(road_point_state, [
    1, 10.0 * np.sqrt(2), 0, 0, 0, 0, 1, 0, 0, 0, 1, 11.0 * np.sqrt(2), 0, 0,
    0, 0, 1, 0, 0, 0
],
                           rtol=1e-5,
                           atol=1e-5)
...
for _ in range(5):
    t = time.time()
    new_state = scenario.observation(vehs[0], 120, 2 * np.pi)
    diff = time.time() - t
    times.append(diff)
...
for _ in range(5):
    t = time.time()
    new_state = scenario.observation(vehs[0], 120, 1.58)
    diff = time.time() - t
    times.append(diff)
