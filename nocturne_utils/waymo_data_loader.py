...
def precompute_dataset(from_path, to_path, samples_per_file=5000):
    ...
    precomputed_dataset_path.mkdir(exist_ok=True)
        ...
        for time in range(tmin, tmax + 1):
            for veh in scenario.getVehicles():
                if scenario.hasExpertAction(veh.getID(), time):
                    ...
                    assert (len(veh_state.shape) == 1 and len(expert_action.shape) == 1)
                    
                    # generate (state, action) string        
                    sa_str = ','.join(map(str, veh_state)) + ';' + ','.join(map(str, expert_action))
    ...
    print(f'Finished precomputing dataset, yielding {sample_count} (out of {total_sample_count}) samples.')
    print(f'{s_nan_count} samples ({round(100 * s_nan_count / (s_nan_count + sample_count), 1)}%) were ignored because their state contained NaN.')
    print(f'{a_nan_count} samples ({round(100 * a_nan_count / (a_nan_count + sample_count), 1)}%) were ignored because their action contained NaN.')
...
class WaymoDataset(Dataset):
    def __init__(self, cfg):
        ...
        print(f'Found {len(self.file_paths)} files for a total of {len(self)} samples.')
        print(f'Observation shape is {state_example.shape}, eg: {state_example}')
        print(f'Expert action shape is {action_example.shape}, eg: {action_example}')
    ...
    def __getitem__(self, idx):
        ...
        # load that file in cache (using cache because dataset is pre-shuffled so getitems should be consecutive, ie dont shuffle dataloader)
        ...
        veh_state, expert_action = self._parse_state_action(self.cached_data[1][idx])
        
        return torch.as_tensor(veh_state, dtype=torch.float32), torch.as_tensor(expert_action, dtype=torch.float32)
...
class ImitationAgent(nn.Module):
    ...
    def dist(self, x):
        x_out = self.nn(x)
        m = MultivariateNormal(x_out[:,0:2], torch.diag_embed(torch.exp(x_out[:,2:4])))
        return m
...
if __name__== '__main__':
    ...
    parser.add_argument('--precompute', action='store_true')  # Setting this will erase the whole content of the --to folder! 
    ...
    scheduler = LinearLR(optimizer, start_factor=1.0, end_factor=0.1, total_iters=n_epochs, verbose=True)
    ...
    for epoch in range(n_epochs):
        ...
        losses = []
        for batch, (states, expert_actions) in enumerate(tqdm(train_dataloader, unit='batch')):
