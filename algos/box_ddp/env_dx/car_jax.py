...
def discrete_dynamics(x, u):
    ...
    if hasattr(dx, 'vehicle_pos_indices'):
        for i in range(len(dx.vehicle_heading_indices)):
            ...
            veh_pos_unit_vector = veh_pos_unit_vector.at[dx.vehicle_pos_indices[2 * i]].set(1)
            x = x + veh_pos_unit_vector * dX * dx.dt
            veh_pos_unit_vector = jax.numpy.zeros_like(x)
            veh_pos_unit_vector = veh_pos_unit_vector.at[dx.vehicle_pos_indices[2 * i + 1]].set(1)
            x = x + veh_pos_unit_vector * dY * dx.dt

    # now we recompute all the distances
    # goal dist
    new_goal_dist = jax.numpy.linalg.norm(x[dx.ego_pos_index] - x[dx.goal_pos_index])
    ...
    if hasattr(dx, 'vehicle_pos_indices'):
        for i in range(len(dx.vehicle_heading_indices)):
            new_dist = jax.numpy.linalg.norm(x[dx.ego_pos_index] - x[dx.vehicle_pos_indices[2 * i: 2 * (i + 1)]])
            veh_dist_unit_vector = jax.numpy.zeros_like(x)
            veh_dist_unit_vector = veh_dist_unit_vector.at[dx.vehicle_dist_indices[i]].set(1)
    ...
    for i in range(len(dx.road_dist_indices)):
        ...
        endpoint_1 = jax.numpy.hstack(((road_pos_center[0] + length * jax.numpy.cos(heading) / 2),
                                    (road_pos_center[1] + length * jax.numpy.sin(heading) / 2)))
        endpoint_2 = jax.numpy.hstack(((road_pos_center[0] - length * jax.numpy.cos(heading) / 2),
                                    (road_pos_center[1] - length * jax.numpy.sin(heading) / 2)))
        ego_pos =  x[dx.ego_pos_index]
        # find the closest point on the line and project
        proj_len = jax.numpy.dot((ego_pos - endpoint_1), (endpoint_2 - endpoint_1)) / (length ** 2)
        t = jax.numpy.maximum(jax.numpy.zeros_like(proj_len), jax.numpy.minimum(jax.numpy.ones_like(proj_len), proj_len))
        ...
        road_dist_unit_vector = road_dist_unit_vector.at[dx.road_dist_indices[i]].set(1)
...
def cost_1step(x, u):
    ...
    constraint_cost = jax.numpy.matmul(jax.numpy.diag(mask_vec), jax.numpy.exp(2 * constraint_vec))
...
def cost_trj(x_trj, u_trj):
    total = 0.
    total, x_trj, u_trj = jax.lax.fori_loop(0, TIME_STEPS-1, cost_trj_looper, [total, x_trj, u_trj])
...
def backward_pass(x_trj, u_trj):
    ...
    for i in range(TIME_STEPS - 1):
        V_x, V_xx, k_trj, K_trj, x_trj, u_trj, expected_cost_redu, n_qp_iter = backward_pass_looper(i, [V_x, V_xx, k_trj, K_trj, x_trj, u_trj, expected_cost_redu, pnqp_iters])
...
def backward_pass_looper(i, input_):
    ...
    import ipdb; ipdb.set_trace()
    k, Qt_uu_free_LU, If, n_qp_iter = pnqp(
        Q_uu, Q_u, lb, ub,
        x_init=k_trj[n], n_iter=n_iter)
    Q_ux_ = np.array(Q_ux)
    Q_ux_ = np.where((1 - If).tile((*Q_x.shape, 1)).T, np.zeros_like(Q_ux), Q_ux)
    ...
    return [V_x, V_xx, k_trj, K_trj, x_trj, u_trj, expected_cost_redu, n_qp_iter]
...
def run_ilqr_main(x0, u_trj):
    ...
    cost_trace = jax.ops.index_update(
        np.zeros((max_iter+1)), jax.ops.index[0], cost_trj(x_trj, u_trj)
    )
    ...
    for i in range(max_iter):
        x_trj, u_trj, cost_trace = run_ilqr_looper(i, [x_trj, u_trj, cost_trace])
        import ipdb; ipdb.set_trace()
...
def main(cfg):
    ...
    for i in range(run_iter):
        ...
        nominal_states, nominal_actions, nominal_objs = run_ilqr_main(state, u_trj)
        # import ipdb; ipdb.set_trace()
