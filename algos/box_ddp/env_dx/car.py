...
class CarDx(nn.Module):
    ...
    def forward(self, x, u):
        ...
        x = x + heading_unit_vector * dHeading * self.dt
        # speed updates
        ...
        if hasattr(self, 'vehicle_pos_indices'):
            for i in range(len(self.vehicle_pos_indices)):
                heading = x[:, self.vehicle_heading_indices[i]]
        ...
        # now do this for all the object distances
        # TODO(eugenevinitsky) finish this

        return x

    def grad_input(self, x, u):
        '''Computed df / dx, df / du'''
        # we use non-analytic grads for now because...lazy
        pass
...
class VehicleCost(nn.Module):
    ...
    def forward(self, x):
        ...
        cost = torch.matmul(x, torch.matmul(Q, x.T))
