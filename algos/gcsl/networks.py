...
class MultiInputNetwork(nn.Module):
    ...
    def forward(self, *args):
        ...
        embeddings = [
            embed_fn(x) for embed_fn, x in zip(self.input_embeddings, args)
        ]
...
def class_select(logits, target):
    ...
    one_hot_mask = (torch.arange(0,
                                 num_classes).long().repeat(batch_size, 1).eq(
                                     target.data.repeat(num_classes, 1).t()))
    return logits.masked_select(one_hot_mask)


def cross_entropy_with_weights(logits,
                               target,
                               weights=None,
                               label_smoothing=0):
    assert logits.dim() == 2
    ...
    loss = torch.logsumexp(
        logits, dim=1) - (1 - label_smoothing) * class_select(
            logits, target) - label_smoothing * logits.mean(dim=1)
...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(self, input, target, weights=None):
        ce = cross_entropy_with_weights(input, target, weights,
...
class DiscreteStochasticGoalPolicy(nn.Module, policy.GoalConditionedPolicy):
    ...
    def nll(self, obs, goal, actions, horizon=None):
        logits = self.forward(obs, goal, horizon=horizon)
        return CrossEntropyLoss(aggregate=None, label_smoothing=0)(
            logits,
            actions,
            weights=None,
        )
...
class IndependentDiscretizedStochasticGoalPolicy(nn.Module,
    ...
    def nll(self, obs, goal, actions, horizon=None):
        ...
        loss_perdim = CrossEntropyLoss(aggregate=None,
                                       label_smoothing=0)(logits_perdim,
                                                          actions_perdim,
                                                          weights=None)
