...
class ImageandProprio(Box):
    def __init__(self, image_shape, proprio_shape):
        ...
        example = self.to_flat(np.zeros(self.image_shape),
                               np.zeros(self.proprio_shape))
    ...
    def to_flat(self, image, proprio):
        ...
        proprio = proprio.reshape(
            *proprio.shape[:-1 * len(self.proprio_shape)], -1)
    ...
    def __init__(
        ...
        if 'proprio_observation' in spaces.keys():
            ...
            spaces['image_proprio_observation'] = concatenate_box_spaces(
                flat_image_space, spaces['proprio_observation'])
            spaces['image_proprio_desired_goal'] = concatenate_box_spaces(
                flat_image_space, spaces['proprio_desired_goal'])
            spaces['image_proprio_achieved_goal'] = concatenate_box_spaces(
                flat_image_space, spaces['proprio_achieved_goal'])
        ...
        if self._presampled_goals is None:
            ...
            self.num_goals_presampled = presampled_goals[np.random.choice(
    ...
    def _update_obs(self, obs):
        ...
        if self.return_image_proprio:
            obs['image_proprio_observation'] = np.concatenate(
                (obs['image_observation'].flatten(),
                 obs['proprio_observation']))
            obs['image_proprio_desired_goal'] = np.concatenate(
                (obs['image_desired_goal'].flatten(),
                 obs['proprio_desired_goal']))
            obs['image_proprio_achieved_goal'] = np.concatenate(
                (obs['image_achieved_goal'].flatten(),
                 obs['proprio_achieved_goal']))
