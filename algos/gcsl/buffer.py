...
class ReplayBuffer:
    ...
    def __init__(
        ...
        self._desired_states = np.zeros((buffer_size, *env.state_space.shape),
                                        dtype=env.state_space.dtype)
        ...
        self._length_of_traj = np.zeros((buffer_size, ), dtype=int)
        self.pointer = 0
    ...
    def _sample_indices(self, batch_size):
        ...
        time_idxs_1 = np.floor(
            prop_idxs_1 * (self._length_of_traj[traj_idxs] - 1)).astype(int)
        time_idxs_2 = np.floor(prop_idxs_2 *
                               (self._length_of_traj[traj_idxs])).astype(int)
    ...
    def sample_batch(self, batch_size):
        ...
        traj_idxs, time_state_idxs, time_goal_idxs = self._sample_indices(
            batch_size)
    ...
    def load(self, file_name, replace=False):
        ...
        states, actions, desired_states = data['states'], data[
            'actions'], data['desired_states']
    ...
    def state_dict(self):
        d = dict(
            internal_goals=self._internal_goals[:self.current_buffer_size])
        if self._states.shape[2] < 100:  # Not images
            d.update(
                dict(
                    states=self._states[:self.current_buffer_size],
                    actions=self._actions[:self.current_buffer_size],
                    desired_states=self._desired_states[:self.
                                                        current_buffer_size],
                ))
        return d
